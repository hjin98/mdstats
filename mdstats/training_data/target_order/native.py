"""Private exact native execution backend for MVSEL2 candidate-row scoring.

Restored unchanged in semantics from carrier ``3937881e`` blob ``d38eda61``.
...
def qualify_mvsel2_native_backend() -> MVSEL2NativeBackendStatus:
...
def mvsel2_execution_backend(workers: int) -> str:
    workers = int(workers)
    if workers < 1:
        raise TrainingDataInputError(
            "MVSEL2 worker count must be positive."
        )
    if workers == 1:
    ...
    if not status.available or not status.qualified:
        raise TrainingDataInputError(
            "MVSEL2 workers>1 requires the qualified native "
            f"backend; {status.reason or 'backend unavailable'}. Build the "
            "package native extensions before running the parallel selector."
        )
    if not status.openmp:
        raise TrainingDataInputError(
            "MVSEL2 native backend was built without OpenMP; "
            "workers>1 is unavailable."
        )
    return "native-openmp"


def score_family_candidate_batch(
    offsets: Any,
    witnesses: Any,
    terms: Any,
    candidates: Any,
    *,
    ...
    if not status.available or not status.qualified:
        raise TrainingDataInputError(
            f"MVSEL2 native backend is not qualified: {status.reason}"
        )
    if workers > 1 and not status.openmp:
        raise TrainingDataInputError(
            "MVSEL2 native backend has no OpenMP support."
        )
    ...
    if (
        ...
        raise TrainingDataInputError(
            "MVSEL2 native offsets must be contiguous uint64."
        )
    if (
        ...
        raise TrainingDataInputError(
            "MVSEL2 native witnesses must be contiguous uint32."
        )
    if (
        terms_array.ndim != 1
        ...
        raise TrainingDataInputError(
            "MVSEL2 native terms must be contiguous float64."
        )
    ...
    try:
        ...
        raise TrainingDataInputError(
            f"MVSEL2 native family scoring failed: {exc}"
        ) from exc
    return output, edges
