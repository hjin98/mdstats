...
@dataclass(frozen=True, slots=True)
class Eval2TargetRole:
    """Leakage-safe target evaluation role used by EVAL2.

    Target-size screening/final-development checkpoint ranking may not consume
    the outer monitor.  For those cases EVAL2 uses a frozen complement of the
    largest authorized training rung inside TARGET-DATA2A development.  CV runs
    use their already-frozen checkpoint-monitor units.
    """
    ...
    serialization_schema: str = field(default=EVAL2_TARGET_ROLE_SCHEMA, repr=False, compare=False)

    def __post_init__(self) -> None:
        if self.serialization_schema != EVAL2_TARGET_ROLE_SCHEMA:
            raise TrainingDataInputError("Unsupported EVAL2 target-role schema.")
        if not str(self.label_domain_id).strip():
            raise TrainingDataInputError("EVAL2 target role requires a label-domain ID.")
        ...
        if not frames or len(frames) != len(set(frames)) or len(blocks) != len(frames):
            raise TrainingDataInputError("EVAL2 target role requires unique evaluation frames and one block ID per frame.")
        if len(excluded) != len(set(excluded)) or set(frames) & set(excluded):
            raise TrainingDataInputError("EVAL2 target evaluation and excluded training memberships must be disjoint.")
        ...
        object.__setattr__(self, "fold_index", None if self.fold_index is None else int(self.fold_index))
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "Eval2TargetRole":
        if payload.get("schema") != EVAL2_TARGET_ROLE_SCHEMA:
            raise TrainingDataSerializationError("Unsupported EVAL2 target-role schema.")
        result = cls(
        ...
        _validate_record_digest(payload, result.content_digest, name="EVAL2 target role")
        return result


def _development_frame_to_block(role_domain: Any) -> dict[str, str]:
    result: dict[str, str] = {}
    for interval in role_domain.development_intervals:
        for frame_uid in interval.frame_uids:
            if frame_uid in result:
                raise TrainingDataInputError("EVAL2 development intervals overlap in frame membership.")
            result[frame_uid] = interval.unit_id
    return result
...
def build_eval2_coarse_size_study_target_role(
    ...
    *,
    ...
    Every candidate size in the coarse screen therefore sees exactly the same
    target frames while inference cost remains bounded.
    """

    limit = int(maximum_configurations)
    if limit <= 0:
    ...
    if len(full.evaluation_frame_uids) <= limit:
        ...
        selected_blocks = full.correlation_block_ids
    else:
        ordered_blocks: list[str] = []
        block_members: dict[str, list[str]] = {}
        for uid, block in zip(full.evaluation_frame_uids, full.correlation_block_ids, strict=True):
            if block not in block_members:
                ordered_blocks.append(block)
                block_members[block] = []
            block_members[block].append(uid)
        quotas = {block: 0 for block in ordered_blocks}
        remaining = limit
        active = list(ordered_blocks)
        while remaining > 0 and active:
            next_active: list[str] = []
            for block in active:
                if remaining <= 0:
                    break
                if quotas[block] < len(block_members[block]):
                    quotas[block] += 1
                    remaining -= 1
                if quotas[block] < len(block_members[block]):
                    next_active.append(block)
            active = next_active
        chosen: set[str] = set()
        for block in ordered_blocks:
            ...
            indices = []
            for i in range(q):
                index = min(len(members) - 1, int((i + 0.5) * len(members) / q))
                while index in indices and index + 1 < len(members):
                    index += 1
                while index in indices and index > 0:
                    index -= 1
                indices.append(index)
            chosen.update(members[i] for i in sorted(indices))
        selected_uids = tuple(uid for uid in full.evaluation_frame_uids if uid in chosen)
        selected_blocks = tuple(
            block for uid, block in zip(full.evaluation_frame_uids, full.correlation_block_ids, strict=True)
            if uid in chosen
        )
        if len(selected_uids) != limit:
            raise TrainingDataInputError("EVAL2 coarse monitor sampler failed to realize the requested deterministic size.")
    return Eval2TargetRole(
...
def build_eval2_cv_target_role(
    role_freeze: Any,
    *,
    label_domain_id: str,
    fold_index: int,
) -> Eval2TargetRole:
    """Freeze the existing DATA5 CV checkpoint-monitor role for EVAL2 ranking."""

    domain = role_freeze.domain(label_domain_id)
    mapping = dict(domain.cv_checkpoint_monitor_unit_ids_by_fold)
    if int(fold_index) not in mapping:
        raise TrainingDataInputError("EVAL2 CV target role references an unknown fold.")
    units = set(mapping[int(fold_index)])
    frames: list[str] = []
    blocks: list[str] = []
    for interval in domain.development_intervals:
        if interval.unit_id in units:
            frames.extend(interval.frame_uids)
            blocks.extend([interval.unit_id] * len(interval.frame_uids))
    if not frames:
        raise TrainingDataInputError("EVAL2 CV checkpoint-monitor role is empty.")
    return Eval2TargetRole(
        label_domain_id=label_domain_id,
