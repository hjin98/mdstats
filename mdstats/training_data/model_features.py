...
def _validate_mace_candidate_architecture(
    ...
    # The architecture record names the loss family the reconstructed model is
    # built for, and it must be the family that actually executes.  Pinned MACE
    # derives ``compute_stress`` for this family exactly as it did for the
    # retired ``universal`` one, so reconstruction and EVAL2 semantics are
    # unchanged; what changes is that model identity no longer disagrees with
