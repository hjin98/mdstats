...
from mdstats.plotting.density_scene_planning import plan_density_support_atlas
...
def run(a):
    ...
    for field in fields:
        ...
        t=time.perf_counter(); pp=plan_density_support_atlas(src,rt); print('[stage] preplan', time.perf_counter()-t, flush=True); t=time.perf_counter(); at=build_density_support_atlas(src,rt); atlas_s=time.perf_counter()-t; print('[stage] build',atlas_s,flush=True)
