...
def run(a):
    ...
    for field in fields:
        ...
        print('[stage] resolution done', resolution, flush=True); t=time.perf_counter(); cic=aggregate_periodic_cic_sparse_optimized(samples,res.grid_shape); cic_s=time.perf_counter()-t; print('[stage] cic done', cic_s, flush=True)
        t=time.perf_counter(); st,_=get_periodic_gaussian_stencil_support(res.grid_shape,scene.display_cell,res.gaussian_bandwidth,kernel_tail_tolerance=a.kernel_tail_tolerance); stencil_s=time.perf_counter()-t; print('[stage] stencil done', stencil_s, flush=True)
        t=time.perf_counter(); src=pack_periodic_cic_source(cic,storage_block_shape=tuple(a.block_shape)); source_s=time.perf_counter()-t; print('[stage] pack done', source_s, flush=True)
        t=time.perf_counter(); rt,_=get_periodic_kernel_block_routing(st,storage_block_shape=tuple(a.block_shape)); routing_s=time.perf_counter()-t; print('[stage] route done', routing_s, flush=True)
        t=time.perf_counter(); at=build_density_support_atlas(src,rt); atlas_s=time.perf_counter()-t; print('[stage] atlas done', atlas_s, at.metadata.to_json_dict().get('dilation_backend'), flush=True)
        ...
        t=time.perf_counter(); plan=plan_hybrid_tiled_realization(src,st,rt,at,options=opt); plan_s=time.perf_counter()-t; print('[stage] plan done', plan_s, flush=True)
