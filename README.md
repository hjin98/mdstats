...
Run `mdstats-mlff-campaign --config campaign.toml storage report` to inventory the campaign workspace without reclaiming anything (bare `storage` is the same report). All storage management lives under this one command: `storage cleanup --tier safe|cache`, `storage deduplicate`, and `storage archive create|list|verify|restore|reclaim`. The owner-driven reset superseded the STOR1-STOR5 tier policy described here: eligibility now comes from the real P1-P7 owners, `--tier archive` no longer exists (cold archival is `storage archive`), and the retired `recompute`/`compact` tiers are rejected by name. The report is bounded, is printed rather than written into `results/`, and every observational command leaves the campaign byte-for-byte as it found it. `storage report --deep` remains the explicit exact recursive physical accounting. See `docs/specs/training_data/mlff_storage_management_spec.md` for the current normative contract.
