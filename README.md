...
Current development release: **0.20.212a0**. The frozen single-source replay migration is implemented additively through `REPLAY-UNIFY1C`: one selected replay source, canonical geometry/split authority, source-true-label cache/materialization, and foundation-prediction/audit/qualification/lazy pseudo-label views are complete. Live TRAIN2/DATA8 campaign execution intentionally remains on the historical split-file replay path until `REPLAY-UNIFY1D`; therefore the previous FINAL-GPU1 workstation handoff remains archival and no positive GPU qualification is claimed by this release. The target-data MVMIGRATE1 v5/v3/v3 activation likewise remains gated on the regenerated final one-shot GPU evidence.
