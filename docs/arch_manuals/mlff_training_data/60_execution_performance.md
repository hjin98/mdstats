...
Target-size screening is treated as a distinct reuse topology. REPAIR2 candidate prefixes are authenticated/materialized in bulk once per unique label-domain/size pair, and the fixed pre-selection evaluation complement is derived once from the maximum qualified prefix. The expensive DATA7 fitted metric/E0/weight core is selection-size invariant, so target-size variants may reuse that core through a reconstructible execution-only index to a fully authenticated DATA7 carrier artifact. Size-specific selection and coverage are then realized normally, and the resulting full `Data7PreparationBundle` remains the sole scientific authority. The shared full-artifact recipe is v2 and excludes DATA8-only evaluation membership/target-study outcome state while retaining the exact prescribed training prefix and selection policy. Legacy v1 recipes remain read-compatible.
