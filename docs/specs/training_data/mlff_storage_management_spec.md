...
That authority stays root-bound all the way to the mutation. A certification performed on a directory is not reduced to a set of typed names plus a fresh generic pathname walk before a consequential action: the released-attempt view carries the `(device, inode)` the owner observed, the common member resolver re-acquires the namespace through the same strict descent and requires that identity to match, an identity change stales an unapplied plan, and the final recursive removal re-observes the root immediately before entry. `shutil.rmtree` avoiding symlink attacks is not proof that a separately authenticated root *name* was not replaced by a same-shaped directory; where the proof cannot be preserved the subtree is retained rather than removed.
