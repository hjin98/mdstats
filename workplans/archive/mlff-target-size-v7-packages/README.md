...
One narrower blocker was recorded against executable `84868ecc...`: the shared mutable component-position reader treated **any** parseable payload lacking `position_object` as the old direct representation, so a malformed current `mdstats.qualification-component-position-locator.v1` could delete that mandatory field, fall through the legacy branch, and let unauthenticated coordination bytes choose which authentic evidence `qualification status` reported.

That defect is now repaired. `read_component_position()` discriminates the two representations explicitly: a current-schema locator must carry its complete contract, authenticate the immutable position object it names, and agree with that object's own component/binding/input/evidence claims; the pre-locator direct payload is recognized by an explicit shape rule and validated on its own terms; everything else, including a current locator with a mandatory field deleted, degrades to `unreadable_position` with a blocked detail. The repair stayed inside the one existing shared reader -- no position registry, CAS promotion, second observer, cache, batch policy, or storage mutation path was added, and the parent target-size scientific question, P5 CV/final-production science, P7 qualification science, CampaignStore's current-authority role, the generation-safe prepared/frame design, and Storage Revision 38's canonical destructive architecture are all untouched. Repair detail, focused acceptance, structural caller closure, and the affected-surface regression comparison against an unmodified-head worktree are recorded in `../MLFF_CAMPAIGN_P1_P7_STORAGE_INTEGRATION_IMPLEMENTATION_PROGRESS.md`.
