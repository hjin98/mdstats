...
Gate A may receive **PASS FOR HUMAN RATIFICATION** only if:
...
6. **Residual-reserve limitation.** Confirm that `M3` is explicitly a finite development/model-selection reserve chosen while training support has priority and is not claimed to be an unbiased estimator of a broader physical/deployment population.
...
There is no predetermined percentage threshold to manufacture a pass. Assess whether the baseline is numerically/scientifically stable enough for the intended target-size experiment or whether D1/D2 needs a different accepted weighting/uncertainty treatment.

## 6. Representative CPU/RAM feasibility evidence

The exact split/completion semantics are mandatory; no approximate fallback is admissible for Gate-A closure.
...
- `m3`, `N_max`, and `K=max(candidate_sizes)`;
- active target-order coordinate dimension;
- exact-solver memoized/explored state count and exact pruning used;
...
Wall time is evidence, not an arbitrary scientific threshold. The question is whether the exact method is operationally feasible for the supported regime with reasonable bounded CPU/RAM resources. If not, reopen D2 rather than weaken semantics.

Production-scale GPU qualification remains intentionally deferred to the established final-release phase unless a future accepted method unexpectedly introduces GPU-dependent numerical semantics.

## 7. Capability-transfer and losslessness audit
