...
entry_condition: independent PASS for P6 revision 11 and P7 revision 7, followed by binding to the accepted post-P7 commit/tree
---

# Storage/I-O reset package authority — revision 6
...
```text
CODE-MLFF-TARGET-SIZE-V7-P6 revision 11 -> independent cleanup/cutover PASS
CODE-MLFF-TARGET-SIZE-V7-P7 revision 7 -> independent publication/qualification PASS
```
...
P6 revision 11 deliberately keeps transitional storage maximally conservative: `safe|cache` remains the public vocabulary, but neither tier evicts current cache families; `frame-cache`, `checkpoint-model-cache`, and uncertified historical cache families are retained/deferred.  P6 does not invent cache leases, current-consumer registries, or a parallel `workspace/runs` cache authority merely to reclaim bytes.  P7 revision 7 then adds the final publication/qualification owner surface. Neither predecessor implements this storage reset.
