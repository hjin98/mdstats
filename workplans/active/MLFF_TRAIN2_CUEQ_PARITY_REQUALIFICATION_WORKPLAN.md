...
workplan_review_status: PASS_AS_WORKPLAN_AFTER_STAGE_B_SECOND_AUTHOR_CHALLENGE_REPAIR
workplan_review_basis: 3da1cd1fb9c45d3d60d15ef84c028543bec0a25d
...
8. Test the **training-state transition**, not merely raw calculator outputs or an implementation-specific gradient coordinate. For each active TRAIN2 objective/head branch and bound model state, reset exact model/optimizer/EMA/RNG state, execute the minimum two optimizer updates needed to populate and then consume stateful optimizer recurrence, map both resulting states through the independently qualified canonical state-transfer transform into one portable e3nn coordinate/function representation, and compare update-induced e3nn prediction changes on a frozen witness corpus. The measurement transform must not re-import source/DATA6 descriptor/FPS acceptance into TRAIN2. Step 1 and step 2 transition deltas are separate governed vectors.
...
13. Keep non-finite values, shape/head/objective/runtime mismatch, projection/authentication mismatch and unsupported applicability as hard fail-closed outcomes. Add a candidate-independent catastrophic guard at the accepted TRAIN2 robust-loss property scale: no retained paired backend discrepancy in energy/atom, force, or stress (including transition-displacement channels) may reach the corresponding `0.01` property Huber transition scale. This is a gross materiality guard, not the equivalence tolerance.
