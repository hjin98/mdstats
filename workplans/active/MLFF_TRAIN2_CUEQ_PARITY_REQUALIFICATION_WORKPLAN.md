...
workplan_review_status: PASS_AS_WORKPLAN_AFTER_STAGE_B_AUTHOR_CHALLENGE_REPAIR
workplan_review_basis: eb6c1138fd872a41ac53bdc9b768de29d0d8b56d
...
6. Separate **persistent bias** from stochastic non-determinism. A systematic backend centroid shift cannot borrow a one-realization noise RMS as its acceptance margin because systematic training error can accumulate while zero-mean execution noise need not. Global and order-cell centroids must instead satisfy the pre-existing dtype mixed numerical envelope componentwise: FP32 `rtol=1e-5, atol=1e-6`; FP64 `rtol=1e-10, atol=1e-12`. These values are fixed independently of the Stage-A CuEq outcomes.
7. Use e3nn-only repeatability only for the stochastic-spread coordinate. For each governed vector derive a tolerance RMS scale from the dtype mixed envelope around the e3nn centroid, set the reference stochastic budget to the larger of e3nn observed variance and that fixed tolerance scale, and require CuEq total/global and cell-local variance to be at most twice that reference budget. The factor-two variance budget is predeclared and means candidate stochastic spread may not consume more than one additional reference-sized variance budget.
8. Test the **training-state transition**, not merely raw calculator outputs or an implementation-specific gradient coordinate. For each active TRAIN2 objective/head branch and bound model state, reset exact model/optimizer/EMA/RNG state, execute the minimum two optimizer updates needed to populate and then consume stateful optimizer recurrence, project both resulting states through the already-governed canonical portable e3nn representation, and compare the update-induced e3nn prediction changes on a frozen witness corpus. Step 1 and step 2 transition deltas are separate governed vectors.
9. Require gradients, losses, parameters, optimizer state and EMA state to remain finite during the two-step witness. The transition witness must use the real TRAIN2 objective/exposure and every active branch that can reach the optimizer. A descriptor-only or calculator-only proxy cannot substitute for this operator consequence.
...
13. Keep non-finite values, shape/head/objective/runtime mismatch, projection/authentication mismatch and unsupported applicability as hard fail-closed outcomes.
...
- physical forward-channel centroids and two-step update-induced portable-e3nn prediction deltas under the frozen finite-sample functional, with finite loss/gradient/optimizer/EMA execution during the witness;
- an evidence corpus adequate for the claimed validity domain;
- synthetic/adversarial counterexamples from Section 4.6, including candidate variance inflation, cell-specific persistent-bias cancellation, optimizer-state-step-2 drift, descriptor-only drift, wrong head/objective/state/runtime, projection corruption, and deterministic-reference/tolerance-floor cases;
