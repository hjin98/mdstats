...
Candidate 8 is the semantic repair of independent Review R4 NO-PASS for immutable Candidate 7.

Candidate 7 remains immutable historical proposed authority. Candidate 8 does not reinterpret or mutate Candidate 7 and uses no Candidate-7 Stage-C result; none exists.
...
Candidate 8 repairs the R4 defects by changing the proposed D2 relation rather than adding an implementation exception:

1. the former derived \`Gamma -> eta -> p_max\` rule is removed;
2. stochastic noninferiority slack and catastrophic population risk are explicit stakeholder-ratified method coordinates with no generated defaults;
3. child launch order is an i.i.d. qualification-only randomized nuisance, distinct from production arithmetic identity, restoring the exact Bernoulli model used by Clopper-Pearson;
4. continuous adequacy is owned only by exact role-effective downstream consumers with source-available accepted materiality/equivalence relations; a consumer without such a relation makes that role unqualifiable rather than minting a tolerance;
5. complete portable model state remains structurally inventoried and projection-authenticated, but raw parameter-state ULP magnitude is no longer misrepresented as scientific materiality;
6. the broad-displacement score is an exact unnormalized ULP mass, so unchanged coordinates cannot dilute it and storage block partitioning cannot tune it; and
7. production projection coefficients must equal the correctly rounded independently derived semantic transform before output-error bounds are considered.

Candidate 8 is a parameterized D2 method family. Two risk coordinates and the prospective evidence cardinality have no default and MUST be explicitly bound by stakeholder ratification of the exact Candidate-8 method instance before Stage C:

- \(\eta_{\rm NI}\): allowed excess strict-worse probability above the exchangeability baseline;
- \(q_{\rm cat}\): allowed fresh-process probability of a joint source-defined materiality violation; and
- \(n\): prospective triplet count.

Admissible bindings satisfy

\[
0\le \eta_{\rm NI}\le q_{\rm cat}<0.10.
\]

The strict \(q_{\rm cat}<0.10\) ceiling is a falsification constraint: a method instance may not accept the Review-R3 adversary whose reference method has materially bad continuous excursions with probability \(0.10\). It is not a claim that any value near \(0.10\) is desirable. The exact lower risk budget is a stakeholder decision.

Candidate 8 remains proposed until fresh independent Review PASS and exact stakeholder ratification. A PASS does not accept CuEq, does not create Stage-C evidence, and does not authorize D3/D4.

## 2. Exact realization and protected consequence

### D2.CUEQ8.DEF.001 — authorizing realization key
...
- \(\rho\) binds the common runtime/hardware context plus every arithmetic-relevant backend-specific runtime, determinism, TF32/matmul, library, device, construction, and execution coordinate needed to interpret \(K_R,K_C\);
- \(m\) is immutable Candidate-8 method identity; and
- \(R=(\eta_{\rm NI},q_{\rm cat},n,q_{\rm design})\) is the ratified qualification-risk/evidence-design tuple.
...
Candidate 8 fixes simultaneous confidence target
...
so zero observed joint materiality events can support the ratified population-risk ceiling. Larger \(n\) is allowed only if fixed before Candidate-8 execution for an explicit evidence-budget/power reason; outcome-selected sample-size increase is forbidden.

A qualification applies only to its exact bound key. Another corpus, scientific seed, horizon, objective, initial state, topology, consumer population, runtime, device class, backend/kernel pair, risk tuple, or material arithmetic state is another key.
...
### D2.CUEQ8.DEF.002 — chosen closure path: exact downstream consumer population

Candidate 8 chooses the second repair route permitted by Review R3: it binds and observes the complete actual downstream continuous-consumer population for the exact qualified role rather than claiming raw stochastic equality of every parameter coordinate.
...
Raw portable parameter/state ULP magnitude is not a standalone scientific-materiality claim in Candidate 8. Complete state is structurally protected by inventory/projection identity; numerical adequacy is owned by the complete actual consumer consequence \(E\).
...
Backend-specific representation layout and kernels may differ only where Candidate 8 explicitly treats them as the deliberate acceleration coordinate.

### D2.CUEQ8.DEF.004 — complete accepted loader
...
Candidate 8 does not import P3 target-size complete-batch drop_last=false semantics into P5.

A distributed route is outside applicability until separately accepted.

### D2.CUEQ8.DEF.005 — full horizon
...
### D2.CUEQ8.DEF.006 — portable coordinate
...
### D2.CUEQ8.DEF.007 — complete forward-affecting inventory
...
### D2.CUEQ8.DEF.008 — state boundaries and consumer activation
...
Candidate 8 does not compress raw state-value difference into an authorizing stochastic score. Instead, each state boundary at which the accepted role consumes a continuous quantity is evaluated through the portable e3nn provider, and the final completed state is always evaluated on exact \(E\) before authorization.
...
### D2.CUEQ8.DEF.009 — independence contract
...
### D2.CUEQ8.DEF.010 — exact inventory reconciliation
...
### D2.CUEQ8.DEF.011 — semantic transform ownership and coefficient qualification
...
Let \(\widehat A\) be the actual production coefficient matrix. Before any output bound is evaluated, Candidate 8 requires exact bit-pattern equality
...
### D2.CUEQ8.DEF.013 — source-closed consumer trace
...
A continuous quantity with no accepted materiality/equivalence/decision-geometry relation is not assigned a new tolerance by Candidate 8. The corresponding role is unqualifiable until its owning authority supplies one or the role is narrowed so the quantity is not a governed consumer.

For a pair of realizations, convert each same-dtype floating coordinate difference to exact ULP distance \(z_i\) using D2.CUEQ8.DEF.012.

The ordered trace identity and cardinality are bound before outcomes.

### D2.CUEQ8.DEF.014 — rare and broad discrepancy scores without dimensional dilution
...
Scientific/material adequacy is not inferred from these ULP scores alone; it is enforced independently by D2.CUEQ8.DEF.017-019.

## 8. Independent triplet design

### D2.CUEQ8.DEF.015 — independent experimental unit and qualification-only order randomization

The independent experimental unit is one replicate triplet \(p\).

Each triplet contains three freshly constructed OS-process trajectories under the same exact scientific key:

- \(R1_p\): accepted e3nn;
- \(R2_p\): accepted e3nn;
- \(C_p\): pure CuEq.

No child process reuses mutable model, optimizer, EMA, RNG, CUDA graph, loader, calculator, or other backend state from another child.

Across triplets, mutable process state is fresh. Scientific RNG/loader seed identity remains fixed by \(A\) where the accepted method requires it.

The order in which the three **independent child processes are launched** is a qualification-only nuisance and is not production arithmetic identity. For every triplet, draw one of the six permutations independently and uniformly using dedicated design RNG \(q_{\rm design}\). This design RNG cannot enter any child scientific RNG, loader order, model initialization, or training arithmetic.

Therefore, even if launch-order strata have different conditional score distributions, each triplet is an i.i.d. draw from the same prospectively declared uniform six-stratum mixture. This is the population to which the exact binomial confidence statements below apply.

Production-internal construction/evaluation order that can change arithmetic is not randomized here; it remains bound inside \(K_R,K_C,\rho\) and another production order is another key.

No warm-up, retry, discarded triplet, or order reassignment may depend on observed score/outcome. If process isolation or the i.i.d. qualification-only mixture cannot be established on the target host, qualification fails closed.

### D2.CUEQ8.DEF.016 — triplet scores
...
\(R2\) and \(C\) are not asserted independent of the common anchor \(R1\). Under the exact backend-equivalence null, conditional on \(R1\) and the sampled nuisance stratum, \(R2\) and \(C\) must be exchangeable for the protected consumer-score object. Sharing \(R1\) therefore does not invalidate the paired strict-worse event.

Separate triplets are the independent Bernoulli units.

## 9. Reference adequacy

### D2.CUEQ8.DEF.017 — source-available continuous materiality relation

For every governed continuous consumer \(e\), Candidate 8 requires an exact accepted source relation
...
### D2.CUEQ8.DEF.019 — joint reference/candidate materiality events and population risk
...
Candidate 8 requires **zero** observed \(C^R_p\) and **zero** observed \(C^C_p\) over the predeclared \(n\) triplets.

For either event population, zero failures in \(n\) i.i.d. triplets gives the exact one-sided Clopper-Pearson upper bound
...
Thus reference continuous adequacy and candidate catastrophic-tail risk are explicit population propositions, not sample-maximum anecdotes.

A reference method with materially bad continuous behavior at probability \(0.10\) is outside every admissible Candidate-8 instance because \(q_{\rm cat}<0.10\). A rare severe candidate event is governed by the same joint population-risk ceiling regardless of its magnitude.

## 10. Systematic-shift noninferiority relation

### D2.CUEQ8.DEF.020 — ratified noninferiority coordinate

Candidate 8 removes the Candidate-7 derivation
...
Instead, \(\eta_{\rm NI}\) is an explicit stakeholder-ratified D2 method coordinate with no default. Its meaning is exactly the allowed excess probability, above the exchangeability baseline, that the candidate consumer-score realization is strictly worse than reference self.
...
This coupling is conservative: the numerical noninferiority slack cannot exceed the entire ratified probability budget for a source-defined materiality failure. It is not inferred from confidence level, \(\Gamma\), machine epsilon, or Candidate observations.

### D2.CUEQ8.DEF.021 — strict-worse event and exchangeability
...
Under exact exchangeability of \(R2\) and \(C\) conditional on the common \(R1\) and the sampled launch-order nuisance,

\[
\Pr(S^{RC}_{p,j}>S^{RR}_{p,j})
=
\Pr(S^{RR}_{p,j}>S^{RC}_{p,j})
\le\frac12
\]

even for discrete/tied distributions.

The uniform independently randomized nuisance mixture in D2.CUEQ8.DEF.015 preserves one common marginal Bernoulli probability across triplets.

### D2.CUEQ8.DEF.022 — exact finite-sample confidence
...
For each \(j\), let \(U_j\) be the exact one-sided Clopper-Pearson upper confidence bound for a Binomial\((n,p_j)\) worse probability with per-statement error \(\alpha=0.0125\).

Candidate 8 requires
...
By the four-way Bonferroni allocation in D2.CUEQ8.DEF.001, the two systematic-shift statements plus the reference/candidate materiality-risk statements hold simultaneously with confidence at least \(0.95\).

No Candidate-8 observation may alter \(\eta_{\rm NI}\), \(q_{\rm cat}\), \(n\), score definitions, materiality relations, launch-order distribution, or confidence allocation.

This is a genuine noninferiority claim with an explicitly ratified margin; it is no longer presented as a theorem derived from an unrelated reference-content target.

## 11. Catastrophic-tail semantics

### D2.CUEQ8.DEF.023 — observed-sample fail-fast guard

D2.CUEQ8.DEF.019 is the population catastrophic-risk authority.
...
### D2.CUEQ8.DEF.024 — no tolerance from zero/reference extrema
...
### D2.CUEQ8.DEF.025 — discrete preservation
...
### D2.CUEQ8.DEF.026 — latent optimizer state
...
Candidate 8 does not require elementwise cross-basis equality of \(q_{\rm opt}\).
...
### D2.CUEQ8.DEF.027 — no cross-backend restart
...
### D2.CUEQ8.DEF.028 — source/DATA6

Candidate 8 does not promote any generic CuEq source/DATA6 relation.
...
### D2.CUEQ8.DEF.030 — projection relation

Only a completed state produced by a current Candidate-8-qualified TRAIN2 realization may enter completed-state projection.

Projection requires:

1. exact completed CuEq state authentication;
2. immutable snapshot input;
3. D2.CUEQ8.DEF.007 complete portable-forward inventory;
4. dependency-native production projection;
5. D2.CUEQ8.DEF.009-011 independent structural/coefficient oracle;
...
### D2.CUEQ8.DEF.031 — projection evaluator population
...
The evaluator uses the same Candidate-8 definitions D2.CUEQ8.DEF.015-024, with its own exact ratified \(n_{\rm eval}\) and design RNG identity. Its \(\eta_{\rm NI}\) and \(q_{\rm cat}\) cannot be looser than the TRAIN2 method instance.

Because child launch order is independently randomized under the same six-permutation mixture, the binomial model is explicit rather than inferred from a fixed counterbalanced cycle.

Evaluator qualification never substitutes for structural projection proof.

### D2.CUEQ8.DEF.032 — EVAL2 identity
...
### D2.CUEQ8.DEF.033 — authorizing record

An authorizing record binds at least:

- immutable Candidate-8 commit/blob/digest;
- exact accepted parent identities;
- exact \(A\);
- ratified \(\eta_{\rm NI},q_{\rm cat},n,q_{\rm design}\);
...
- ULP rank version;
- exact qualification-only launch-order RNG and realized permutations;
...
Candidate 7 and earlier evidence cannot authorize Candidate 8. Stage-A evidence remains method-design evidence only.

## 17. Routine doctor

### D2.CUEQ8.DEF.034 — cheap currentness/reachability witness
...
1. immutable Candidate 8 receives fresh independent D2 Review PASS; and
2. the stakeholder ratifies the exact Candidate-8 method instance, including \(\eta_{\rm NI},q_{\rm cat},n\) and the exact consumer/materiality source set.
...
3. complete accepted TRAIN2 horizon for every triplet child;
4. the predeclared \(n\) triplets with i.i.d. uniform six-permutation qualification-only launch randomization;
5. exact source-owner resolution for every \(\mathcal R_e\);
6. zero observed joint reference materiality failures;
7. zero observed joint candidate materiality failures;
8. exact population upper bounds no larger than ratified \(q_{\rm cat}\);
9. \(S_{\max}\) and exact-integer \(S_{\Sigma}\) collection over the canonical consumer trace;
10. exact Clopper-Pearson systematic-shift bounds no larger than \(1/2+\eta_{\rm NI}\);
11. exact scientific-decision equality;
12. injected final-step hidden-state defect that affects an \(E\) consumer and must fail;
13. injected large decision-stable continuous reference noise outside a source-owned relation and must produce REFERENCE_METHOD_UNSTABLE;
14. injected rare severe candidate consumer violation and must count as the joint catastrophic event;
15. storage/tensor repartition with identical consumer trace and therefore identical \(S_{\Sigma}\);
16. same-backend restart/resume when claimed;
17. attempted cross-backend restart, which must fail closed;
18. stale qualification after any bound-key/risk/materiality-source change;
19. attempted FP64 CuEq TRAIN2, which must fail closed;
20. attempted source/DATA6 CuEq authorization from TRAIN2 evidence, which must fail closed;
21. projection omission/duplicate/wrong-permutation/wrong-k-range/quiescent-state injections, which must fail structurally;
22. wrong production projection coefficient including scalar \(A^\ast=1,\widehat A=100\), which must fail before evaluator comparison;
23. independently derived correctly rounded coefficient fixtures;
24. ill-conditioned/non-unique semantic transform, which must fail closed;
25. underflow/subnormal/overflow/fused-reduction adversaries for the realized projection kernel; and
26. evaluator-agreement attempts after structural failure, which must remain failures.

No Candidate-7 or Candidate-8 outcome may be used to choose the ratified risk coordinates, consumer relation, score definition, transform definition, process count, or launch distribution.

## 19. R3/R4 counterexample closure

| Counterexample | Candidate-8 treatment |
|---|---|
...
| fixed six-order cycle violates binomial model | replaced by i.i.d. uniform qualification-only launch permutation |
| shared \(R1\) anchor | valid under conditional R2/C exchangeability; explicitly retained |
...
| correlated converter/oracle helpers | forbidden as in Candidate 7 |
...
| generic publication outside \(E\) inferred | outside Candidate-8 authority |
| source/DATA6 CuEq inferred | explicitly forbidden |
| FP64 TRAIN2 inferred | explicitly forbidden |

Candidate 8 deliberately narrows open-ended publication authority rather than manufacture a generic state-space materiality tolerance.

## 20. Author-side Challenge and handoff state

Author-side falsification after the R4 repair found no remaining known D2 false-pass route inside the declared Candidate-8 family.

The most important change is conceptual: Candidate 8 no longer asks an unowned ULP magnitude on model parameters to stand in for scientific materiality. The complete portable state remains structurally accountable, while actual continuous adequacy is defined only at consumers whose accepted owner provides a relation.
...
The projection repair is intentionally strict. Correctly rounded semantic coefficients are required before learned-dtype accumulation error is considered. If pinned MACE 0.3.16 cannot satisfy that relation, Candidate 8 fails Stage C rather than widening the coefficient allowance.

No Candidate-8 Stage-C result exists.

Candidate 8 must now be frozen immutably and subjected to a genuinely fresh independent D2 Review. A fresh Review must challenge the parameterized-risk ownership itself, the i.i.d. launch-mixture argument, source availability of every \(\mathcal R_e\), the consumer-set closure, and the independent semantic definition/correct rounding of \(A^\ast\).
