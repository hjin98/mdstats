...
**P5 revision 5 is implemented and awaiting independent review.** Its evidence record is `P5_REVISION5_IMPLEMENTATION_PROGRESS.md`. The cumulative revision-2/3/4/5 contract is realized in the new current post-selection owners, the `cross-validate` and `train-production` public commands, and 77 real-owner acceptance tests plus affected P1-P4/CLI/EVAL2 regression. P5 is **not** formally accepted and P6 remains blocked until independent review passes.

**P5 revision 5 contract summary.** Revision 5 preserves the complete revision-2/revision-3/revision-4 P4-to-P5, currentness, CV, replay, leakage, and fresh-production contract while normalizing the last identity-layer ambiguity into an acyclic `policy -> plan -> realized evidence` graph. Shared method, CV-validation policy, and final-production policy identities contain only canonical policy/configuration and stable recipe definitions; exact `T_selected`/P1 relation projections/fold memberships bind CV plans; accepted CV and frozen M3 lineage bind final-production plans; fitted DATA7/DATA8/TRAIN2/EVAL2/checkpoint/acceptance products remain descendants and never redefine parent policy digests.
...
P1 neutral scientific substrate
  ...
  -> P5 revision 5 post-selection CV and fresh final production
...
- P5 CV consumes the complete canonical P1 split-exclusion/protected relation authority projected onto exact `T_selected`; correlation units and geometry duplicates are mandatory but not exhaustive, and unselected related siblings never enlarge the selected dataset.
...
- P5 CV and final production share one scientific **method** identity, not one monolithic role-specific policy digest. CV fold/monitor/budget policy and final-production horizon/final-run policy have separate downstream policy identities.
- P5 downstream identity is acyclic: policy identities contain only canonical configuration/stable recipes; exact selected/P1/fold/M3/current-CV lineage belongs to immutable plans; fitted/materialized/TRAIN2/EVAL2/checkpoint/acceptance products belong to descendant evidence and cannot redefine policy or plan identity.
- CV-only settings invalidate CV descendants/current authorization only and do not mutate P4 target-size state. Production-only `[training].max_num_epochs`/adaptive changes invalidate affected final-production descendants only and do not require CV rerun.
- Every required CV fold and required CV seed/variant must satisfy the configured target-only outer-validation predicate. Aggregate means, majority folds/seeds, best-seed shortcuts, replay-weighted scores, and partial-fold evidence cannot authorize production.
- P5 replay TRUE_DFT evidence is admissibility/safety evidence only. Among admissible checkpoints/seeds, ordering is target-only; legacy target+replay weighted combined scores are not current ranking or acceptance authority.
- Cross-fold/cross-seed dispersion and replay summaries remain diagnostic unless a future governing scientific revision explicitly promotes them to a gate.
- P5 final target gradients use full exact `T_selected`; frozen M3 may serve only as development/model-selection target evidence and must not be relabeled independent validation. M3 lineage binds the final-production plan/evidence, not the production-policy digest.
- P5 must not feed CV evidence or configuration back into target-size selection.
- P5 final production starts fresh; its production horizon is the resolved `[training].max_num_epochs` and is independent of target-size screening `n3` and the CV budget.
...
5. `P5_POST_SELECTION_CV_FINAL_PRODUCTION.md` — revision-5 active implementation contract: cumulative revision-2/revision-3/revision-4 currentness/CV/replay/fresh-production hardening plus final acyclic policy -> plan -> realized-evidence identity normalization.
