...
**Repair state:** B1-B3 repairs are implemented in the working candidate and await independent re-review. B1 consolidated the threshold semantics into the broad canonical owners (D1 `mlff_scientific_method.md` §10.3/§11, D2 `mlff_numerical_algorithmic_method.md` §17.1/§23.7, D3 `arch_manuals/mlff_training_data/40_training_evaluation.md`, D4 `mlff_post_selection_p5_spec.md` §12.1), marked D1/D2 accepted, and removed the four parallel threshold-delta authority files. B2 made the existing `_finite_positive_threshold` owner reject booleans/strings and passes the raw `[acceptance]` value to it. B3 replaced the confounded assembled oracle with a `tau_cv`-only edit. The cycle stays reopened until that re-review passes.
