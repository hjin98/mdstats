...
Revision 10 is **PASS AS WORKPLAN / D3->D4 handoff ready after final exhaustive convergence review**. The frozen design separates P5 scientific member selection from a subordinate full-MACE-model publication, materializes the exact selected checkpoint rather than trainer terminal output, publishes the decision/model/reclosure pointer set atomically, uses descriptor/no-follow trust and no-clobber immutable artifacts with recoverable fresh locators, makes `train-production` print the authoritative model path/SHA/head directly, and preserves independent per-size products without introducing a cross-size winner. P7 keeps checkpoint reconstruction as scientific reference while consuming authenticated P5 full-model bytes only for deployment; deployment-dependent evidence binds the exact model-artifact set, first locked reveal is fenced against binding/model drift, release exposure requires the matching release index, and deployed ML-IAP artifacts receive the same receipt/byte trust discipline. Storage ownership, lifecycle observation and current documentation are explicitly reconciled. MH-1 remains lightweight compatibility-only; long real campaign/GPU qualification stays deferred.
