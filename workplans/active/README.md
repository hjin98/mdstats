...
Revision 22 is **PASS AS WORKPLAN / D3->D4 handoff ready after final handoff-discoverability review**. The frozen design keeps the existing P5 scientific publication decision unchanged and adds one subordinate full-MACE-model publication for the exact selected representative checkpoint(s), never the trainer terminal model. Publication uses one decision-set lock, immutable no-clobber model artifacts under `models/production/`, one atomic final-decision/model/reclosure pointer transaction, exact selected-state/head/dtype/architecture authentication, create-or-verify recovery, direct `train-production` model path/SHA/head output, and independent per-size products with no cross-size release winner.

Currentness is now explicit end to end: public P5 observation snapshots CV/final-plan pointers plus the complete dynamic final-seed assessment-position parent graph and proves each seed at its exact canonical locator key; P7 admission consumes one coherent captured P5 parent graph and keeps checkpoint reconstruction as the scientific reference while using the authenticated P5 full model only for deployment. Deployment parity and dynamics share one invocation-frozen ordered ML-IAP realization set, so different deployed bytes cannot be mixed into one terminal verdict. Attempt-local deployment receipts/artifacts remain reuse/execution scratch rather than public currentness authority; released-scratch cleanup therefore cannot invalidate durable release evidence. First locked reveal remains one-shot and is fenced against current binding/P5-parent drift without making the cohort identity representation-specific. Storage ownership, no-follow/descriptor trust, fsync/no-clobber durability, runtime-compatibility reclosure, exact owner reuse, lifecycle/status coherence, and current documentation are all in the acceptance contract. MH-1 remains lightweight compatibility-only; long real campaign/GPU qualification stays deferred.
