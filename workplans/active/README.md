...
- The replay repair is governed jointly by `MLFF_REPLAY_TRUE_DFT_DEFAULT_PSEUDOLABEL_PREP_AND_CUDA_LIFETIME_REPAIR_WORKPLAN.md`, `MLFF_REPLAY_TRUE_DFT_DEFAULT_PSEUDOLABEL_PREP_AND_CUDA_LIFETIME_SECOND_REVIEW_AMENDMENT.md`, and `MLFF_REPLAY_TRUE_DFT_DEFAULT_PSEUDOLABEL_PREP_AND_CUDA_LIFETIME_SECOND_REVIEW_FINAL_CLOSURE.md`. The final closure is the latest review result and controls where the three conflict. Together they govern the single-source TRUE_DFT default, prepare-owned replay preparation/currentness, exact current-alias replacement, storage-safe replay lineage, lifecycle/P5 currentness under an unchanged target binding, concurrent cold-build safety, and pseudo-label provider/executor CUDA lifetime.
