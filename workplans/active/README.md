...
- The replay repair is governed jointly by `MLFF_REPLAY_TRUE_DFT_DEFAULT_PSEUDOLABEL_PREP_AND_CUDA_LIFETIME_REPAIR_WORKPLAN.md` and `MLFF_REPLAY_TRUE_DFT_DEFAULT_PSEUDOLABEL_PREP_AND_CUDA_LIFETIME_SECOND_REVIEW_AMENDMENT.md`. The amendment is the latest review closure and controls where the two conflict. Together they govern the single-source TRUE_DFT default, prepare-owned replay preparation/currentness, storage-safe replay lineage, concurrent cold-build safety, and pseudo-label provider/executor CUDA lifetime.
