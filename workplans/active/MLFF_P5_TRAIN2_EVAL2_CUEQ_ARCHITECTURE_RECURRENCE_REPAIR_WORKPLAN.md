...
## 13. Implementation status (2026-09-13, after review reopen)

**Disposition:** D4 repairs for H1, H2, and the portable projection are implemented; no Serious Challenge. The 2026-09-12 D3 challenge (§10 trigger 3) is **withdrawn**: the independent review amendment `MLFF_P5_TRAIN2_EVAL2_CUEQ_ARCHITECTURE_RECURRENCE_INDEPENDENT_REVIEW_REOPEN.md` (R1) required falsifying pinned MACE's native `transfer_weights` into the canonical portable shell first, and that route preserves both the portable architecture and the accepted numerical semantics. Ready for a new independent review; the H1/H2 repairs, strict digest gates, and failure-path release are unchanged.
...
- **Sibling at A/G.** For every foundation-backed CuEq method (naive and multihead, FP32 and FP64; scratch round-trips exactly), native `convert_cueq_e3nn.run` rebuilds the target from `extract_config`, so `radial_embedding.bessel_fn.bessel_weights` (a Parameter after `load_foundations_elements`) becomes a buffer and CG `U_matrix` buffers are recomputed (≤1.2e-7 float32 rounding, same basis, deterministic per path). The projection therefore cannot equal the configuration's canonical portable architecture under the strict R17A descriptor. MACE's own saved TRAIN2 `.model` differs from our projection only in one U-matrix value (3e-8). Resolved at D4: `transfer_weights` into the canonical shell instead of `run`'s rebuilt shell (R1 evidence below).

### D4 changes

- `model_features.realize_mace_training_model`: device via pinned `mace.tools.init_device`; converter frames are dropped on failure.
- `model_features.restore_mace_portable_model(realized, portable, config)`: for CuEq it transfers authenticated state **into the canonical portable shell the realization was built from** through pinned `convert_cueq_e3nn.transfer_weights`, with converter parameters from pinned `extract_config_mace_model`. The converter-created-shell branch (`convert_cueq_e3nn.run`) is removed from production; OEq keeps its native `run` (not the qualified path; no evidence it needs the change).
- `evaluation.authenticate_train2_checkpoint_provider`: the canonical portable digest is fixed before the in-place projection and compared exactly after it; on mismatch the diagnostic rebuilds the canonical shell (failure path only) for `first_difference`.
...
- `tests/test_mlff_p5_train2_eval2_cueq_realization_parity.py` (CUDA, 17 cases, no xfail): A == B; C == D == F; dtype/replay/foundation/scratch-normalization rejections before state load; R1 projection for foundation FP32/FP64 and scratch FP32 × live/EMA-like state — digest unchanged, every portable parameter equal to pinned `convert_cueq_e3nn.run` state, `compare_mace_acceleration_calculators` passed under `MaceAccelerationParityPolicy`; production authentication → bounded `run_bounded_inference` matching the CuEq model at FP32 policy tolerance; injected post-projection drift rejected with `first_difference=interaction_avg_num_neighbors`; non-accumulating CUDA residency on rejected and accepted paths; `only_cueq=true` performs no conversion either way. 9 fail on the pre-change owners (the drift-rejection and rejection-before-load cases guard preserved behaviour and pass on both).
- G8 `test_p5_real_cross_validate_resumes_eval2_for_replay_only_elements[e3nn|cueq]`: real CLI `cross-validate` → qualified wrapper → pinned `run_train` (CuEq variant: `enable_cueq=true`, `only_cueq=false`, CUDA) with replay-only H → EVAL2 interruption → resume through production authentication, canonical projection, and the real provider batch forward on every chunk (statistics keep the bounded harness values) with zero TRAIN2 relaunches. The CuEq variant fails on the pre-change owners with the stakeholder's `round-trip ... first_difference=parameters`.
- G5/R3 (read-only, production `authenticate_post_selection_provider`, EMA): all 5 stakeholder runs authenticate at the latest checkpoint (companion) and at epoch 4 (earlier boundary), reach one exact portable digest, and give finite real forwards; the campaign database is byte-unchanged. Real stakeholder `cross-validate` (TRAIN2 launch forbidden by a raising trainer; DB and runs backed up first): 0 TRAIN2 launches, recovery reused 5/5 runs, EVAL2 authenticated and evaluated run `262e…`, then stopped with the typed scientific outcome `No CV fold checkpoint passed mandatory admissibility: replay_retention_ceiling_exceeded, target_threshold_exceeded`. Independent oracle: the epoch-9 EMA EVAL2 provider matches MACE's own saved `.model` to ≤2.8e-5 eV/Å on target and replay heads; target force RMSE vs DFT on 6 monitor frames is 0.075 eV/Å against the frozen 0.030 maximum. The inadmissibility is the trained method's outcome (non-goal of this workplan), not a projection defect.
- Affected regression: see §13 closeout below.

### Closeout

- Final affected regression on one unchanged candidate (83 files that reach TRAIN2/EVAL2 authentication, P3 realized architecture, P5 runtime/recovery/admission, target-size EVAL2, qualification providers, or the assembled campaign CLI; `-n 16`): **1930 passed, 2 skipped, 1 failed, 0 xfailed**. The failure, `tests/test_mlff_target_size_p5f_structure.py::test_p5f_no_screening_continuation_owner_is_reachable_from_post_selection` (`restart_latest` referenced from `post_selection_execution.py`), fails identically at accepted baseline `b65fa3b0`, and both files are byte-identical to that baseline: pre-existing, unrelated.
- Structural: a Semgrep rule (positive control: pinned `run_train.py`) finds no production import/call of `convert_cueq_e3nn.run` and no two-argument `restore_mace_portable_model` call. Ruff `F,E9`: the same 2 pre-existing findings before and after.
- Governing invariants §3.2 and forbidden strategies §4: no digest substitution, tolerance, descriptor narrowing, alternate portable identity, checkpoint wrapper, or backend fallback; the post-projection check stays exact against the digest fixed before projection.
- Documentation (G9): no D3 change, so permanent architecture/specification documents are unchanged. PEM candidate overlay: NT-001 updated and kept REVIEW_REQUIRED pending independent review.
- Unavailable: production-scale GPU qualification (not claimed). OEq portable projection is unchanged and unqualified for foundation-backed models.
- Stakeholder action outside this workplan: fold 1 of the real CV is scientifically inadmissible under the frozen thresholds. A backup taken before the real run is at `05_mace_training/LTA/mpa0/FP32/.r3-backup-2026-09-13/` and can be deleted once the new EVAL2 records are accepted.
- Next: request a new independent SSDP 6.3 review.
