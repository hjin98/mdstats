...
status: PROPOSED_R2_REVIEW_CANDIDATE
...
supersedes_proposed_candidates:
  ...
  - workplans/active/MLFF_D2_SSDP64_AXIOMATIC_AUTHORITY_R2_FINAL.md
---

# Proposed MLFF D2 formal numerical authority kernel — R2 review candidate

## 1. Authority and exact import registry

This is the sole D2 file in the R2 semantic candidate. Every earlier D2 candidate listed above is authoring history only and does not compose with this candidate.

The candidate formalizes, without changing, accepted MLFF D2 at repository `hjin98/mdstats` basis `cb07d68372f1b6d25f9e8b62a2fb7e31fb82e824`. It remains proposed until fresh independent R2 review passes and the stakeholder ratifies the exact reviewed candidate.
...
For finite values `v_i` with D2.DEF.013 stored weights, stable-sort by `(v_i, original_index)`. For governed `q in [0,1)`, `Q(q)` is the first sorted value whose cumulative stored weight is at least `q`; if no such value exists, preparation fails. The stored weights are never normalized again inside this definition.
...
Automatic P3 screening requires `|Q_cfg|>=3`; otherwise `INSUFFICIENT_AUTOMATIC_COMPARISON`. No membership or ladder is changed.
...
### D2.DEF.052 — Replay lineage

Replay lineage binds replay geometry/source membership/split, label mode, true-reference monitor, foundation/head where applicable, replay-head E0, prediction policy, and realized exposure. Changing a component invalidates dependent P5 evidence.
...
For checkpoint `c`, `S(c)` is exact shared mandatory constraint conjunction imported from `D2.IMP.CV`: finite required metrics, replay degradation within accepted budget using authenticated true-reference replay monitor when replay active, plus required physical/integrity gates. Pseudo labels cannot satisfy true-reference retention.
...
iff throughout declared validity domain all outputs/decisions in `O` satisfy the equality/tolerance owned by their canonical definition. Selector rank/repair trace may require exact identity; metrics may use separately governed tolerance. Performance does not establish equivalence.
...
## 17. R2 falsification oracles

Fresh R2 must attempt at least: one-normalization 37/2-witness median adversary; selector-vs-MVQUAL tolerance interval adversary; exact component-order/first-predecessor `M3`; condition-balanced `pi_eval`; structural-policy rejection; candidate-common-preparation perturbation; minimum-three-qualified admission; exact funnel/sufficiency/ceiling cases; autocorrelation/block/event fixtures; required-family applicability fixtures; exact adjacency boundaries; full-forward/lazy rank equality; Phase-A tie behavior; REPAIR2 frontier/trace/limits/rank inheritance/no-extra-shell; direct/MVIDX mass and MVQUAL monotonicity; E0 null-space transfer; robust P5 dimensional/nine-stress/mask/no-head-scalar; true-vs-pseudo replay geometry invariance and true retention; replay-first seeded exposure/drop-last; monitor/fold reconstruction; role-threshold boundaries/selective invalidation; and worker/backend/restart invariance.

The binary64 R1 adversary is binding: two represented correlation units with 37 and 2 witnesses produce stored post-normalization sum `1.0000000000000002`; stable cumulative after witness index 36 is `0.5000000000000006`; `Q(0.5)` must choose index 36. Re-normalizing again and choosing 37 is nonconformant.

## 18. D2 -> D3 handoff and acceptance
...
Fresh independent R2 review must show lossless equivalence to accepted basis, R1 B1-B8 closure, exact source availability for every imported prerequisite, definition-before-substantive-use closure, acyclic direct dependency trace, and renderer-safe formulas. Any changed numerical result/decision, tolerance, failure classification, family/applicability set, stochastic exposure, or replay lineage is a semantic change requiring adjudication rather than editorial acceptance.
