...
**Status:** active — Integration & Verification Review R3 is **NO-PASS** on 2026-09-14 against branch head `c5fc4e09a2e83c879f07d0ca3e3c611975f78d32`, explicitly excluding still-open G13 from that verdict. G0/G1/G1A remain CLOSED/PASS; the narrow G1B D4 ancestry repair remains applied with independent re-review pending. One additional D4 scientific/numerical conformance blocker is now open: current replay transports can preserve externally supplied non-binary per-property weight metadata into foundation-P5 native `UniversalLoss`. The P5-F restart oracle and several current specification/guide/config/P7/DATA8 documentation surfaces are also stale. Prior candidate-bound G11/G12 evidence remains historical evidence for unaffected surfaces but is not closure evidence for the newly changed G12A repair surface. G13 pilot/three-fold CV, independent re-review, and closeout remain open.
...
### G12A — replay-transport and integration reconciliation — **OPEN / BLOCKING**
