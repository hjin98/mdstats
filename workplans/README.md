...
Use `TEMPLATE.md` for new workplans. Active plans live in `active/`. After a transition is accepted, move durable current-state consequences into architecture/specifications, move chronology/evidence to their permanent homes, then archive the plan in `archive/` when retaining it is useful for development lineage.
